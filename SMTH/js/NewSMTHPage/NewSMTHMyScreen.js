...
export default class NewSMTHMyScreen extends Component {
    ...
    network() {
        NetworkManager.getNewSMTHAjaxSession((result) => {
            this.setState({
                isLoading: false,
                id: result['id'],
                username: result['id'],
                ...
                first_login: result['first_login_time'],
                last_login: result['last_login_time'],

                mailCount: result['new_mail'],
                sentMailCount: 0,
                likeCount: result['new_like'],
                replyMeCount: result['new_reply'],
                atMeCount: result['new_at'],
            });
        }, (error) => {
                                ...
                                onPress={() => {
                                    if (this.state.login == true) {
                                        AsyncStorageManger.getID().then(id => {
                                            ReactNavigation.navigate(this.props.navigation, 'newUserScreen', { id: null, name: this.state.username });
                                        });
                                    }
                                    else {
                                        this.showLogin();
                        ...
                        this.state.login == true && this.state.gender != null ?
                          ...
                          null
                            ...
                            <View style={[styles.userInfoView, { marginTop: 10, height: 40 }]} >
                                ...
                                <Text style={[styles.userInfoBigText]} >
                                    {this.state.login == true && this.state.life != null ? this.state.life + '(' + this.state.life + ')' : '无'}
                                </Text>
                            </View>
                                ...
                                <View style={{
                                    ...
                                    >
                                        <View style={[styles.messageItem]}>
                                            <View style={{ alignItems: 'center' }}>
                                                ...
                                                <Text style={styles.messageTextItem}>发件箱</Text>
                                                {
                                    ...
                                    <CellBackground
                                        ...
                                        onPress={() => {
                                            if (this.state.login == true) {
                                                // StatusBar.setBarStyle('dark-content');
                                                ReactNavigation.navigate(this.props.navigation, 'newMessageScreen', { selectedIndex: 4 })
                                            }
                                            else {
                                                this.showLogin();
                                            }
...
var styles = {
    ...
    get messageItem() {
        return {
            ...
            width: (global.constants.ScreenWidth - 40) / 5,
