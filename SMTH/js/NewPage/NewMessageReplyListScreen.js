...
export default class NewMessageReplyListScreen extends Component {
    ...
    constructor(props) {
        ...
        this.page = 1;
        this.getNewSMTHReply(this.page);
    }

    getNewSMTHReply(page) {
        NetworkManager.getNewSMTHReply(page, (result) => {
    ...
    _renderItem = ({ item }) => {
        return (
            <CellBackground
                onPress={() => {
                    ReactNavigation.navigate(this.props.navigation, 'newSMTHThreadSingleDetailScreen', { id: item.id, board: item.board_id, title: '回复我' })
                }}
            >
                <View>
